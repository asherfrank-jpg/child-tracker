...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"'Heebo',sans-serif", direction:"rtl" }}>
      ...
      {screen==="login" && (
        <LoginScreen onDone={handleFirebaseLogin} />
      )}
      ...
      {screen==="profile-setup" && (
        <LoginScreen
          isProfileSetup={true}
          onDone={async (p) => {
            await handleUpdateProfile({ childName: p.name, childGender: p.gender, childAvatar: p.avatar });
            setScreen("dashboard");
          }}
        />
      )}

      {screen !== "login" && screen !== "notif-permission" && screen !== "profile-setup" && (
        <>
          {screen==="dashboard" && (
            <DashboardScreen
              profile={activeProfile || { name: (user && user.displayName) || "הורה", avatar:"🧒" }}
          ...
          {screen==="medications" && (
            <MedicationsScreen
              profile={activeProfile || { name: (user && user.displayName) || "הורה" }}
              medications={medications}
              setMedications={handleSetMedications}
            />
          )}
